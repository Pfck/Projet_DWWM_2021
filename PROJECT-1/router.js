...
router.post("/register", signupValidation, (req, res, next) => {
  db.query(
    `SELECT * FROM admin WHERE LOWER(email) = LOWER(${db.escape(
      req.body.email
    )});`,
    (err, result) => {
      if (result.length) {
...
router.post("/formulaire", loginValidation, (req, res, next) => {
  db.query(
    ...
    (err, result) => {
      ...
      // check password
      bcrypt.compare(req.body.mdp, result[0]["password"], (bErr, bResult) => {
        // wrong password
        if (bErr) {
          throw bErr;
          return res.status(401).send({
